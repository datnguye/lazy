...
The best code is the code never written. `lazy` holds an agent to that: YAGNI,
stdlib first, native over dependencies, the shortest diff that works. Authored
once, rendered into nine agent formats. Writing the same prompt nine times
would be, of all things, not lazy.
